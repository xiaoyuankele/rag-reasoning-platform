...
- 混合检索：保留为后续候选能力；当前不实现，先建立真实问题测试集并形成对比报告。
...
本阶段不开发混合检索。接下来只建立中英文真实问题测试集，分别运行关键词检索和语义检索，记录
命中来源、排名、人工相关性与响应时间。完成评估报告后，混合检索仍留在后续计划中，不自动进入
开发队列；只有证据显示两路结果存在稳定且有价值的互补性时，才重新讨论融合算法、分数归一化和
接口契约。详细执行方法见 `docs/evaluation/retrieval-quality-evaluation-plan.md`。
...
10. 使用中英文真实问题建立检索测试集；
11. 形成关键词与语义检索的质量对比报告；
12. 将混合检索保留为后续候选能力，当前不开发；满足重新评估条件后再单独立项。
