...
> 状态：实施中；B1、B2 以及 B3 验证码申请、注册和登录接口已完成，下一步实现 Session 鉴权中间件、当前用户与退出。交接日期：2026-08-16。
> 产品边界、主线图和跨端验收标准以
...
撤销当前 Session、清除 Cookie并返回 `204`。重复退出保持幂等，不泄露 Session 是否曾经存在。

### `GET /users/me`

返回 `200` 和 `{"user": {...}}`。未登录、Session 已过期或已撤销统一返回：
...
后端完成 B3 后即可交付登录态联调；完成 B5 和双用户集成测试后，才可声明多人数据安全边界完成。
