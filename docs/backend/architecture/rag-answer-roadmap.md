...
8. 同步 `/answers` 不存储问答历史；可选异步 `/answer-jobs` 保存任务与单次结果，但不构成多轮会话记忆。
9. `qwen3.6-flash` 第一版默认关闭思考模式，避免推理占满输出预算而没有最终答案；该行为可通过配置调整。
10. 完整问答链路使用进程内 Owner 公平有界调度；默认全局执行 10、单用户执行 2、全局等待 500、
    单用户等待 5，最多同步等待 5 秒。单用户等待预算用尽返回稳定 `429
    answer_owner_capacity_exhausted`；全局等待区已满或等待超时返回稳定 `503
    answer_capacity_exhausted`。调度器只保存等待控制状态，不持久化问题正文。
11. 持久化异步问答默认关闭。启用后，任务状态由 PostgreSQL 保存，Worker 使用 Owner 公平调度领取，
    临时失败执行最多 3 次指数退避，进程退出遗留的 `processing` 在下次启动时恢复为 `queued`。异步 Worker
    复用同步问答的并发包装器，因此两种入口共享全局 10、单用户 2 的最终执行上限。
...
13. [x] 增加可持久化、可轮询、可取消、可启动恢复的异步问答任务，并完成 Owner 隔离和公平领取测试。
14. [x] 增加异步问答排队/执行/端到端耗时、重试恢复和 PostgreSQL 队列压力观测，并纳入 v6 离线报告。
15. [x] 增加异步问答终态数据的 7 天默认保留期、有界批量清理、局部索引和安全清理日志。
16. [x] 增加 100 Owner/200 任务的零远程费用并发回归，覆盖恰好一次执行、并发边界和 shutdown 启动恢复。
17. [x] 增加 Owner 隔离的异步问答任务分页列表，使页面刷新或更换设备后仍可重新发现保留期内的任务。
