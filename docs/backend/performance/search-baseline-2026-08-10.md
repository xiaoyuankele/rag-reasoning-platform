...
含义是较新文档优先，同一文档内保持原文 chunk 顺序，最后用主键消除并列顺序的不确定性。
关键词出现次数、trigram 整体相似度、语义向量和混合召回留到有真实质量样本的后续阶段。

## 最终真实 HTTP 验收（2026-08-11）

正式第 6 号迁移和最终校验和重新对齐后，通过真实 Gin 服务验证：

| 场景 | 结果 |
|---|---|
| `/health` | `status=ok` |
| 英文 `the` | 命中 137；连续两次首批 chunk ID 均为 `159,160,161` |
| 中文“协同控制” | 命中 23；来源文档 ID 为 20 |
| “控制” + `document_id=20` | 命中 30；全部来自文档 20 |
| 字面 `%_` | 命中 0，没有被解释成 SQL 通配符 |

所有请求均返回 HTTP 200。验收结束后服务端口释放；数据库仍为 3 个文档、170 个 chunk、
2 个任务，`schema_migrations` version 6 的校验和与最终迁移文件一致。

至此 P3 第一版完成：它提供可解释、可分页、可过滤、可定位来源且有性能证据的字面关键词
检索。P4 若引入 Embedding 和 RAG，必须另行定义相关性、召回融合和引用准确性标准。
