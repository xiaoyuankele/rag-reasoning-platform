...
P5 不包含用户认证。P6 已完成个人账户、Session、文档归属和全链路数据隔离的设计交接；B1～B5 已交付
身份闭环以及文档、任务、chunks、向量、检索和问答的所有者隔离。B6 又补齐本地 Mailpit 邮件联调与
历史数据受控认领：46 篇文档已归属正式个人用户，关联的 2729 个 chunks、45 个解析任务、8 个向量任务和
460 条向量保持完整。下一步是 B7 `owner_user_id NOT NULL`、发布门禁和最终双用户验收；完成前当前服务
仍不能开放给互不信任的多人；
团队工作区和多租户留到 P7。范围见
