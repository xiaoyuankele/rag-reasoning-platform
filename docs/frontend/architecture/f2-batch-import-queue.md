...
后端并发改造第一阶段不新增批次 API，前端继续使用单文档上传、创建解析任务和任务查询接口。统一 `ApiError` 已能保留
`429`、`503` 和 `Retry-After`，但批量队列尚未实现自动退避、抖动和延迟重试；这些必须等待相关接口冻结稳定 code 后单独开发，
不能把当前通用错误提示写成“已经支持背压恢复”。前端也不应通过提高浏览器并发补偿后端排队。
...
- 自动化覆盖哈希分块/进度/取消、预检真假与降级、上传并发上限、成功解析、ready 重复复用、`409` 恢复、
  单项失败隔离、非法类型和失败重试入口；
