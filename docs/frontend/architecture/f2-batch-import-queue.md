...
> 状态：2026-08-29 已完成批量队列、上传前重复预检、解析任务恢复/逐项取消和容量冷却；真实专项验收见第 7、8 节。
...
它提供 queued 解析任务的逐项取消，但不提供整批服务端取消、批次持久化、批次级审计或批量向量化。
...
| `POST /processing-jobs/latest`     | 创建任务发生 409 时按文档恢复真实任务               |
| `POST /processing-jobs/:id/cancel` | 逐项取消仍为 queued 的解析任务                      |
...
- `ready`：重新读取文档后确认解析完成；
- `canceled`：后端已经确认 queued 任务取消完成，与只停止浏览器操作的 stopped 不同；
...
- 已保存文档继续保留；queued 任务可逐项调用后端取消，processing 任务不能由前端强制终止。

前端上传并发与后端 PDF 处理并发是两套独立容量：浏览器当前默认最多同时上传 2 份文件；后端代码支持 1～4 个
Go Document Worker，代码默认 1，本地开发当前使用 2；Python 执行器可以在 oneshot 与 1～4 个常驻槽位之间切换，
本地开发当前使用 2 个进程池槽位。前端不能读取或镜像这些后端配置，也不能根据自己的上传 worker 数量推断后端处理能力，
更不能把 `queued` 直接显示为“处理中”。

后端并发改造不新增批次上传 API，前端继续使用单文档上传、创建解析任务和任务查询接口。批量队列已识别上传/解析的
owner/global 四个稳定容量 code，保留文件与请求编号，并使用单一 `Retry-After` 冷却阻止提前重复提交；到期只开放
手动重试，不自动重放。指数退避、抖动与自动继续仍未实现，也不通过提高浏览器并发补偿后端排队。
...
- 接入预检后的完整门禁为 23 个测试文件、77 个测试，以及类型检查、ESLint、Prettier 和生产构建通过；
- 自动化覆盖哈希分块/进度/取消、预检真假与降级、上传并发上限、成功解析、ready 重复复用、`409` 最近任务恢复、
  queued 逐项取消、容量冷却、单项失败隔离、非法类型和失败重试入口；
- 已在真实登录态只读打开 `/documents`，确认多文件属性、类型限制、空态、1280px 布局和控制台正常；
- 用户已完成简单真实批次验收：33 份选择只上传 1 份新文件，32 份重复正文被跳过；固定样本 A/B、大文件、
  fail-open、确定性拒绝和双用户隔离仍待专项；
- 若未来要求刷新后恢复整个批次、批次级取消或大量文件长期追踪，再设计后端批次资源。

并发演进的跨端约束集中记录在
[高性能与高并发工程目标](../../shared/architecture/performance-engineering-goal.md)和
