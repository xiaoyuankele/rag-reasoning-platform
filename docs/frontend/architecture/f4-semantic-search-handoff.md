...
隔离保存，用于刷新恢复；该能力默认关闭，只有用户明确勾选才保存，并在 30 分钟、退出登录、Session 失效或密码重置时清除。
问题正文和结果不会写入 URL、Pinia 或跨浏览器会话的永久缓存。
...
- 语义结果刷新恢复：默认关闭，界面明确告知会保存返回的文本片段；用户开启后只使用当前标签页会话缓存，并同时匹配用户 ID 与
  全部/单篇范围；
- 缓存最长保存 30 分钟；关闭选项、退出登录、认证失效或密码重置会立即清除；
...
9. 验证默认不写浏览器存储；开启后立即保存当前成功结果，关闭、超时和匿名化后立即删除。
