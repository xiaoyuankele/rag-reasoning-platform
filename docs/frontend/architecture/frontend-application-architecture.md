...
批量导入进一步把上传编排拆为独立队列：有限并发上传、集中观察任务、逐项失败与重试，并继续以单文档接口和
后端内容哈希判重为真相来源。完整结构与刷新恢复边界见 [F2-B 文档操作闭环](f2b-document-lifecycle.md)，
批量状态机见 [F2 批量导入与解析队列](f2-batch-import-queue.md)。真实批次操作与双用户产品隔离仍待验收。
