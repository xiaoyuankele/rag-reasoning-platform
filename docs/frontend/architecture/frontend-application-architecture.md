...
| 单元/组件测试 | Vitest + Vue Test Utils | 已安装并接入，10 个测试文件、30 个测试通过 | 与 Vite/Vue 集成，验证用户可观察行为 |
...
浏览器表单纵向联调。2026-08-18 已接入 F2 第一批文档实体、运行时 DTO 校验、当前用户分页列表和单文件上传；
前端同时校验 `201 + duplicate:false` 与 `200 + duplicate:true` 的组合，重复内容作为可用的已有记录显示，
不按文件名判断，也不在浏览器承担并发唯一性。真实重复上传与双用户文档产品隔离仍待服务环境启动后验收。
...
- Vitest 验证状态转换、错误转换和纯逻辑；当前 10 个测试文件、30 个测试通过；
