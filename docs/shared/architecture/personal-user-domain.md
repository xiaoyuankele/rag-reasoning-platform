...
> 状态：实施中；B1～B6 已完成。身份闭环和全部用户业务能力已接入 Cookie、OwnerScope 与作用域 SQL；本地 Mailpit 注册和带 dry-run/数量门禁/事务的历史数据认领也已通过真实 PostgreSQL 验收。下一步执行 B7 `owner_user_id NOT NULL` 与发布验收。确认日期：2026-08-17。
...
解析任务、8 个向量任务和 460 条向量仍通过原 `document_id` 关系继承所有者。B7 才负责把这一运行事实
升级为数据库 `NOT NULL` 强约束。
