...
> 当前状态：P1（最小后端骨架，进行中）。Go 模块、Gin 服务、`/health` 接口、PostgreSQL 开发容器、Go 数据库启动连接、文档表迁移、PostgreSQL 文档仓储、文档查询接口、文档上传应用服务和本地文件存储已经完成并验证；HTTP 上传接口、自动迁移执行器和其他文档接口尚未实现。
...
首份 SQL 迁移已建立 `documents` 表，包含主键、文件元数据、处理状态、时间字段、索引和检查约束。该迁移已通过正向执行、合法/非法插入、回滚与重新执行验证；当前仍由 `psql` 手动执行。

文档领域模型已定义与数据库字段对应的 Go 结构体、强类型处理状态和合法状态流转，并已通过表驱动测试验证。

PostgreSQL 文档仓储已实现 `Create` 和 `GetByID`，并通过真实数据库集成测试验证插入、查询、领域级 `ErrNotFound` 转换和测试数据清理。

文档应用服务已实现 `GetByID` 用例，负责校验文档 ID、调用领域仓储，并保留可供 HTTP 层识别的领域错误；相关单元测试和 Go 全量测试均已通过。
...
文档上传应用服务已实现文件保存与数据库记录创建的流程编排，并在数据库写入失败时执行文件补偿删除。本地文件存储已实现 PDF 文件头校验、200 MiB 大小限制、流式写入、SHA-256 计算、临时文件清理、安全路径删除和上下文取消，并通过自动化测试验证；该能力尚未接入 HTTP 路由。

Python 项目、自动迁移执行器、HTTP 文档上传、列表、删除接口和解析链路尚未实现。下一步是把文件存储和上传应用服务接入 Gin multipart 上传接口。
