...
一个面向学生、研究者和小团队的轻量文档知识系统。项目以 Go 构建稳定的业务后端，以 Python 承担多源文档解析、文本分块和后续 AI 能力，优先完成可运行、可测试、可解释的后端主链路。

> 当前状态：P2（异步解析链路，进行中）。文档上传、详情、分页列表和删除接口已经实现并验证；PostgreSQL 自动迁移、解析任务表、任务排队、任务状态查询以及 Worker 的领取与状态收尾已经完成；统一文本块表、领域模型和 PostgreSQL 仓储已经建立。Worker 尚未写入文本块，常驻循环与 Python 解析适配器尚未实现。
...
| AI 与文档处理 | Python | PDF、Markdown、TXT、DOCX 等来源的解析与文本分块，以及后期的向量化、语义检索、摘要和问答 |
...
Worker Application 已定义可替换的文档处理器端口，并实现单次“领取、查询文档、调用处理器、成功或失败收尾”编排。PostgreSQL 会在事务中同步更新任务与文档：成功时分别进入 `succeeded` 和 `ready`，失败时都进入 `failed` 并保存安全错误说明。单元测试和真实 PostgreSQL + Fake Processor 集成测试已经覆盖成功、处理失败、状态回写失败和双重错误保留。

统一文本块领域模型已经区分处理器输入 `ChunkInput` 和持久化结果 `TextChunk`。PostgreSQL 文本块仓储支持在事务中原子替换一份文档的全部文本块，并按块序号查询；真实数据库测试已验证替换、稳定排序、约束失败回滚、缺失文档错误转换和删除文档后的级联清理。

Python 项目和常驻后台 Worker 循环尚未实现。下一步是让文档处理器返回统一文本块结果，并由 Worker 在处理成功后写入 `text_chunks`；随后优先接入简单的 Markdown/TXT 处理器，再逐步增加 PDF 和 DOCX 适配器。
