...
type documentDeleteRepository interface {
	documentdomain.Finder
	documentdomain.Deleter
}
...
func (s *DeleteService) Delete(ctx context.Context, id int64) error {
	if id <= 0 {
		return ErrInvalidID
	}

	foundDocument, err := s.repository.GetByID(ctx, id)
	if err != nil {
